    ...
    public static class MoviesEndpoints
    {
        ...
        public static RouteGroupBuilder MapMovies(this RouteGroupBuilder group)
        {
            ...
            group.MapPost("/createMovie", Create).DisableAntiforgery()
                .AddEndpointFilter<ValidationFilters<CreateMoviesDTO>>();
            group.MapPut("/updateMovie/{id:int}", Update)
                .DisableAntiforgery().AddEndpointFilter<ValidationFilters<CreateMoviesDTO>>(); ;
            ...
            int recordsPerPage = 10)
        {
            ...
            var movieDTO =
                mapper.Map<List<MovieDTO>>(movies); //it will map movies that we have fetched into list having moviedto
            return TypedResults.Ok(movieDTO);
        }

        static async Task<Results<Ok<MovieDTO>, NotFound>> GetById(int id, IMovieRepository movieRepository,
            IMapper mapper)
        {
            ...
            IOutputCacheStore outputCacheStore, IMapper mapper)
        {
            ...
            {
                var url = await fileStorage.Store(containerName,
                    createMoviesDTO.Poster); //store the poster file and get its url
                movie.Poster = url;
            }
            ...
            await outputCacheStore.EvictByTagAsync("movies-get",
                default); //empty the cache with tag "movies-get",purpose of tag is to identify the cache entries related to movies
        ...
        static async Task<Results<NoContent, NotFound>> Update(int id, [FromForm] CreateMoviesDTO createMoviesDTO,
            ...
            , IFileStorage fileStorage, IOutputCacheStore outputCacheStore, IMapper mapper)
        {
            ...
            movieForUpdate.Poster =
                checkMovieExist.Poster; //set the poster of movie entity to the existing poster in database
            if (createMoviesDTO.Poster is not null)
            {
            ...
            IGenreRepository genreRepository)
        {
            ...
            if (genreIds.Count != 0)
            {
                existingGenre = await genreRepository.Exists(genreIds);
            }
